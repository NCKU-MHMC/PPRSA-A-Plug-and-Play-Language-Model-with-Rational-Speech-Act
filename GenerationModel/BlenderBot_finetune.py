# -*- coding: utf-8 -*-
"""
Created on Tue Jan 25 15:51:28 2022
...
def construct_conv(row, tokenizer):
    ...
    for i in range(len(row) - 1):
        if pd.notna(row[i]) and pd.notna(row[i + 1]):
            # inputs.append(tokenizer.encode(row[i], truncation=True, max_length=128) + [tokenizer.eos_token_id])
            inputs.append(tokenizer(row[i] + tokenizer.eos_token, truncation=True, max_length=128))
            outputs.append(tokenizer.encode(row[i + 1], truncation=True, max_length=128) + [tokenizer.eos_token_id])
            
    return inputs, outputs

class ConversationDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, df, block_size=128):
        self.examples = []
        for _, row in df.iterrows():
            inputs, outputs = construct_conv(row, tokenizer)
            for i, o in zip(inputs, outputs):
                self.examples.append((i, o))

    def __len__(self):
        return len(self.examples)

    def __getitem__(self, item):
        input_encode, output_ids = self.examples[item]
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    def collate(examples: List[Dict[str, torch.Tensor]]):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # if inputs.shape[1] > 128: continue
...
def main():
    ...
    df_trn = pd.read_csv(dargs.train_data, header=None)
    df_val = pd.read_csv(dargs.valid_data, header=None)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = BlenderbotSmallForConditionalGeneration.from_pretrained(checkpoint)
            model.to(args.device)
