...
if opt.out_dir != None:
    ...
    path_response = "./output/" + opt.out_dir + "/DialoGPT_medium.txt"
else:
    if not os.path.exists('output'):
        print("===create dir===")
        os.makedirs('output')
    path_response = "./output/DialoGPT_medium.txt"
...
for i in range(len(content_user)):
    ...
    new_user_input_ids = tokenizer.encode(tokenizer.eos_token + content_user[i], return_tensors='pt').to(device)
