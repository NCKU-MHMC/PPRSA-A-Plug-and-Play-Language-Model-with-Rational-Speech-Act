...
np.random.seed(0)
EPSILON = 1e-10
example_sentence = "My brother just turned 16 and he's about to get his first car! I'm so excited for him.<|endoftext|>Whoa that's exciting!  What kind of car we looking at?<|endoftext|>"
example_sentence2 = "My brother just turned 16 and he's about to get his first car! I'm so excited for him.<|endoftext|>Whoa that's bad! Did you suffer any injuries？<|endoftext|>"
...
class Discriminator(torch.nn.Module):
    ...
    def __init__(
        ...
        elif pretrained_model == '../DialoGPT/model-medium':
            self.tokenizer = GPT2Tokenizer.from_pretrained(pretrained_model)
            self.encoder = GPT2LMHeadModel.from_pretrained(pretrained_model)
            self.embed_size = self.encoder.transformer.config.hidden_size
        elif pretrained_model == '../GenerationModel/Llama':
    ...
    def avg_representation(self, x):
        ...
        if self.encoder.config.model_type == 'llama':
            outputs = self.encoder(x, output_hidden_states=True, return_dict=True)
            hidden_states = outputs.hidden_states 
            hidden = hidden_states[-1] # (batch_size, sequence_length, hidden_size)
...
def collate_fn(data):
    def pad_sequences(sequences):
        ...
        for i, seq in enumerate(sequences):
            ...
            # padded_sequences[i, :end] = seq[:end] # right pad
    ...
    with open(dataset_fp,'r', newline='', encoding= 'unicode_escape') as f:
        ...
        for i, row in enumerate(tqdm(csv_reader, ascii=True)):
            if row:
                ...
                text_b = tokenizer.bos_token + row[2].replace('<|endoftext|>','') + tokenizer.eos_token
