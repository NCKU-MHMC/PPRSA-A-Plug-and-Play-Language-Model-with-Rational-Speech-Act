...
DISCRIMINATOR_MODELS_PARAMS = {
    ...
    "Emotion_GO": {
        "path": "../EmotionClassifier_head/output_GOEMO/GO_classifier_head_epoch_6.pt",
        "class_size": 28,
        "embed_size": 1024,
        "pretrained_model": "../DialoGPT/DialoGPT-medium_ftESC",
        "class_vocab": {"grief": 0, "sadness": 1, "admiration": 2, "approval": 3, "confusion": 4,
                        "love": 5, "embarrassment": 6, "nervousness": 7, "remorse": 8, "disappointment": 9,
                        "annoyance": 10, "disapproval": 11, "curiosity": 12, "optimism": 13, "surprise": 14,
                        "pride": 15, "amusement": 16, "joy": 17, "neutral": 18, "anger": 19,
                        "realization": 20, "desire": 21, "disgust": 22, "excitement": 23, "relief": 24,
                        "fear": 25, "caring": 26, "gratitude": 27}
    }
}
...
def generate_text_pplm(
    ...
    for i in range_func:
        ...
        if sample:
            ...
            last = torch.argmax(pert_probs,1).unsqueeze(0)
        ...
        if rsa:
            with torch.no_grad():     
                ...
                speaker_posterior = top_k_filter(speaker_posterior, k=top_k)
                speaker_posterior = F.softmax(speaker_posterior, dim=-1)
                
                if sample:
                    last = torch.multinomial(speaker_posterior, num_samples=1)
                else:
                    last = torch.argmax(speaker_posterior,1).unsqueeze(0)
...
def run_pplm_example(
    ...
    if out_dir != None:
        ...
        if rsa:
            out_name = '/PPRSA_' + attribute_type + '_x' + str(num_iterations)
        else:
            out_name = '/PPLM_' + attribute_type + '_x' + str(num_iterations)
        
        if gold:
            out_name += '_g'
           
        file_pert = open('output/' + out_dir + out_name + '.txt', 'w+', encoding='utf-8')
        file_loss = open('output/' + out_dir + out_name + '_loss.txt', 'w+', encoding='utf-8')
    ...
    if attribute_type == 'all':
        ...
        if gold:
            ...
            intent_prediction_model.load_state_dict(torch.load(intent_prediction_model_file))
