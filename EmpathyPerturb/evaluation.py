...
def classifying_intent(dec_sentence, context_t, model, tokenizer, intent_classifier, device):
    temp = None
    respon_list=[]
    respon_keys=[]
...
def main():
    ...
    for fold_num in range(1):
        ...
        for context_conv, context, response, gold_intents in tqdm(zip(srcs_conv_line, srcs_line, preds_line, pred_intent_line) , mininterval=2, desc='  - (Empathy Evaluation) -  ', leave=False):
            count += 1
            response_ids = [50256] + tokenizer.encode(response) 
            context_conv_ids = tokenizer.encode(context_conv)

            response_full_ids = context_conv_ids + response_ids 
            
            # intent accuracy
            respon_intents = classifying_intent(response, context, model, tokenizer, intent_classifier, device)
