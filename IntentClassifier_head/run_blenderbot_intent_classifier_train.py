...
np.random.seed(0)
EPSILON = 1e-10
example_sentence = "did you think about it ?"
example_sentence2 = "my husband lost a job but i'm hoping he can find a full time job soon"
example_sentence3 = "sorry to hear! do you have any idea about the break up? did you think about it ?"
max_length_seq = 128
...
def predict(input_sentence, model, classes, cached=False, device='cpu'):
    ...
    text1 = input_sentence + "</s>"
    text2 = "<s>" + input_sentence + "</s>"
    ...
    with open(dataset_fp,'r', newline='', encoding= 'unicode_escape') as f:
        ...
        for i, row in enumerate(tqdm(csv_reader, ascii=True)):
            if row:
                label = row[0]
                context = row[1] 
                text = tokenizer.bos_token + row[2]
                
                feature_context = tokenizer(context)
                seq_context = feature_context['input_ids']
                atten = torch.tensor(feature_context['attention_mask'], device=device)
                
                feature_text = tokenizer(text)
                seq_text = feature_text['input_ids']
                
                if (len(seq_context) < max_length_seq) and (len(seq_text) < max_length_seq):
                    seq_context = torch.tensor(
                        seq_context,
                        device=device,
                        dtype=torch.long
                    )
                    seq_text = torch.tensor(
                        seq_text,
                        device=device,
                        dtype=torch.long
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="EDI")
